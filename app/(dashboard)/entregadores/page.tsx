...
import { migrationStoreCouriersWithFallback } from '@/lib/migration-api'
...
export default async function CouriersPage({
  ...
  const courierSnapshot = await migrationStoreCouriersWithFallback(
    ...
    async () => {
      ...
      const [{ data:courierRows, error:courierError }, { data:profileRows, error:profileError }, deliveryResult] = await Promise.all([
        allCourierIds.length
          ? supabase
              .from('couriers')
      ...
      const connected:StoreCourier[] = connectedRows
        .map(network => {
      ...
      const pending:StoreCourierRequest[] = pendingRows.map(network => {
        const item = courierMap.get(network.courier_id)
        const profile = profiles.get(network.courier_id)
  ...
  const couriers = courierSnapshot.connected as StoreCourier[]
  const requests = courierSnapshot.pending as StoreCourierRequest[]
