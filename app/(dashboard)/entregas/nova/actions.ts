...
const n = (v: FormDataEntryValue | null) => { const x = Number(String(v ?? '').replace(',','.')); return Number.isFinite(x) ? x : null }
...
export async function createDeliveryAction(_: CreateState, formData: FormData): Promise<CreateState> {
  ...
  if (!customerName || !deliveryAddress || deliveryFee === null || deliveryFee < 0) return { error:'Preencha cliente, endereço e taxa de entrega.' }
  ...
  revalidatePath('/'); revalidatePath('/entregas')
  redirect('/entregas')
}
